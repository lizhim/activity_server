...
BidList.change_bid_status_starting = function () {
    ...
    var new_bid_array= _.map(bids,function(bid){
        if(bid.user_name == localStorage.getItem("user_name") && bid.activity_name == Activity.get_now_activity_name()
            &&bid.bid_name == BidList.get_biding_name()){
...
BidList.get_bid_array = function () {
    var bids = JSON.parse(localStorage.getItem("bids"));
    var bid_array = _.filter(bids, function (list) {
        return list.user_name == localStorage.getItem("user_name");
    })
    return bid_array;
}